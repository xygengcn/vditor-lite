...
export const mathRender = (
    ...
    if (options.math.engine === "KaTeX") {
        ...
        addScript(
            `${options.cdn}/js/katex/katex.min.js`,
            "vditorKatexScript"
        ).then(() => {
            addScript(
                `${options.cdn}/js/katex/mhchem.min.js`,
                ...
                mathElements.forEach((mathElement) => {
                    ...
                    const math = code160to32(
                        mathRenderAdapter.getCode(mathElement)
                    )
                    ...
                    mathElement.addEventListener(
                        "copy",
                        (event: ClipboardEvent) => {
                            event.stopPropagation()
                            event.preventDefault()
                            const vditorMathElement = (
                                event.currentTarget as HTMLElement
                            ).closest(".language-math")
        ...
        // 循环加载会抛异常
        addScriptSync(
            `${options.cdn}/js/mathjax/tex-svg-full.js`,
