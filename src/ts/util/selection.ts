...
export const getEditorRange = (vditor: IVditor) => {
    let range: Range
    const element = vditor.ir.element
...
export const getCursorPosition = (editor: HTMLElement) => {
    ...
    if (range.getClientRects().length === 0) {
        if (range.startContainer.nodeType === 3) {
            ...
            if (!cursorRect) {
                let parentElement = range.startContainer.childNodes[
                    range.startOffset
                ] as HTMLElement
                while (
...
export const setSelectionByPosition = (
    start: number,
    end: number,
    editor: HTMLElement
) => {
...
export const insertHTML = (html: string, vditor: IVditor) => {
    ...
    if (
        ...
        vditor.ir.element.children.length > 0 &&
    ...
    if (
        ...
        if (!blockElement) {
            vditor.ir.element.insertAdjacentHTML(
                "beforeend",
