...
export const processPasteCode = (html: string, text: string) => {
    ...
    if (
        ...
        (tempElement.lastElementChild as HTMLElement).style.fontFamily.indexOf(
            "monospace"
        ) > -1
    ) {
        // VS Code
...
export const processCodeRender = (
    ...
    const language = previewPanel.firstElementChild.className.replace(
        "language-",
        ""
    )
    if (language === "abc") {
