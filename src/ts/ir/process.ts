...
export const processHint = (vditor: IVditor) => {
    ...
    if (preBeforeElement) {
        if (
            ...
            ;(
                vditor.options.preview.hljs.langs || Constants.CODE_LANGUAGES
            ).forEach((keyName) => {
...
export const processAfterRender = (
    ...
    vditor.ir.processTimeoutId = window.setTimeout(() => {
        ...
        if (vditor.options.counter.enable) {
            let length = text.endsWith("\n") ? text.length - 1 : text.length
            if (vditor.options.counter.type === "text" && vditor.ir) {
                const tempElement = vditor.ir.element.cloneNode(
                    true
                ) as HTMLElement
    ...
    if (actionBtn.classList.contains("vditor-menu--current")) {
        ...
        if (commandName === "line") {
            ...
            if (range.toString() === "") {
                ...
                html = `${prefix}${range.toString()}${suffix.replace(
                    ")",
                    "<wbr>)"
                )}`
            }
